...
from createUser import create_student
...
def getMonthDataAsTable(sel_month):
    data = db.get().val()
    dates = list(data.keys())
    dataframe = pd.DataFrame()
...
def main():
    ...
    if choice == 'Student Login':
        ...
        reg = st.sidebar.text_input('Enter your Register Number')
        ...
        if login:
            ...
            if user['registered'] and user['localId'] == reg:
                ...
                c1, c2 = st.columns(2)
                monthTable = getMonthDataAsTable(selected_month)
                if len(monthTable):
            ...
            if user['registered'] and user['localId'] == "admin":
                ...
                if option == "Add Student":
                    with st.form(key='my_form', clear_on_submit=True):
                        email = st.text_input('Enter Student email Address')
                        password = st.text_input('Enter Student password', type='password')
                        reg = st.text_input('Enter your Register Number')
                    ...
                    if submit:
                        if len(email) <= 0 or len(password) <= 0 or len(reg) <= 0:
                            ...
                            if status:
                                st.success("Student Login Id Created Successfully")
                            else:
        ...
        email = st.sidebar.text_input('Enter your mail Id here')
